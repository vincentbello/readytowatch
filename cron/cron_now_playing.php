<?php // every week. get now playing titles
...
$u = getData("https://api.themoviedb.org/3/movie/now_playing?api_key=$apikey");

if ($u === NULL) {
	echo "Error parsing now playing";
} else {
	$ids = array();
	foreach($u->results as $playing) {
		$ids[] = $playing->id;
	}
